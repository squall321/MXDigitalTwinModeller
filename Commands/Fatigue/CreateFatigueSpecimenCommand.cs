...
namespace SpaceClaim.Api.V252.MXDigitalTwinModeller.Commands.Fatigue
{
    /// <summary>
    /// 피로 시편 생성 커맨드
    /// </summary>
    public class CreateFatigueSpecimenCommand : BaseCommandCapsule
    {
        ...
        public CreateFatigueSpecimenCommand()
            : base(CommandName, "피로시편", IconHelper.FatigueIcon,
                   "피로 시편을 생성합니다 (Fatigue Specimen)")
        {
        }
        ...
        protected override void OnExecute(Command command, ExecutionContext context, Rectangle buttonRect)
        {
            try
            {
                ...
                var dialog = new FatigueSpecimenDialog(part);
                dialog.Show();
            }
            catch (Exception ex)
            {
                ValidationHelper.ShowError(
                    $"피로 시편 생성 중 오류가 발생했습니다:\n\n{ex.Message}", "오류");
            }
        }
    }
}
