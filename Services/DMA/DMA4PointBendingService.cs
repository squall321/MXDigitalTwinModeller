...
namespace SpaceClaim.Api.V252.MXDigitalTwinModeller.Services.DMA
{
    /// <summary>
    /// DMA 4점 굽힘시편 모델링 서비스
    /// SpaceClaim API를 사용하여 4점 굽힘 시편 3D 모델 생성
    /// </summary>
    public class DMA4PointBendingService
    {
        /// <summary>
        /// DMA 4점 굽힘시편 생성 (시편 + 지지구조)
        /// </summary>
        public DesignBody Create4PointBendingSpecimen(Part part, DMA4PointBendingParameters parameters)
        {
            if (part == null)
        ...
        /// 직사각형 바 시편 생성
        /// </summary>
        private DesignBody CreateRectangularBar(Part part, DMA4PointBendingParameters p)
        {
            // mm를 m로 변환
            ...
            // DesignBody 생성
            string name = $"DMA 4-Point Bending Specimen ({p.Length}x{p.Width}x{p.Thickness}mm)";
            return BodyBuilder.CreateDesignBody(part, name, body);
        }

        /// <summary>
        /// 지지 구조 생성 (하부 외부 지지점 2개 + 상부 내부 로딩 노즈 2개)
        /// </summary>
        private void CreateSupportStructure(Part part, DMA4PointBendingParameters p)
        {
            // mm를 m로 변환
            double outerSpan = GeometryUtils.MmToMeters(p.OuterSpan);
            double innerSpan = GeometryUtils.MmToMeters(p.InnerSpan);
            double width = GeometryUtils.MmToMeters(p.Width);
            ...
            // 하부 외부 지지점 위치 (반원 최상단이 시편 바닥 Z=0에 접촉)
            double halfOuterSpan = outerSpan / 2.0;
            ...
            // 하부 외부 지지점 좌 (Left Outer Support Point)
            DesignBody lowerLeftSupport = CreateCylinder(part, "Lower Outer Support (Left)",
                -halfOuterSpan, 0, lowerSupportBaseZ,
                supportDia, width, supportHeight);
            NameSupportFace(lowerLeftSupport, "Left_Lower", true, false);

            // 하부 외부 지지점 우 (Right Outer Support Point)
            DesignBody lowerRightSupport = CreateCylinder(part, "Lower Outer Support (Right)",
                halfOuterSpan, 0, lowerSupportBaseZ,
                supportDia, width, supportHeight);
            NameSupportFace(lowerRightSupport, "Right_Lower", true, false);

            // 상부 내부 로딩 노즈 위치: 반원 최하단이 Z=thickness가 되도록 설정
            double halfInnerSpan = innerSpan / 2.0;
            double upperLoadingBaseZ = thickness + loadingNoseRadius;

            // 상부 내부 로딩 노즈 좌 (Left Inner Loading Nose)
            DesignBody upperLeftLoadingNose = CreateCylinder(part, "Upper Inner Loading Nose (Left)",
                -halfInnerSpan, 0, upperLoadingBaseZ,
                loadingNoseDia, width, loadingNoseHeight);
            NameSupportFace(upperLeftLoadingNose, "Left_Upper", false, true);

            // 상부 내부 로딩 노즈 우 (Right Inner Loading Nose)
            DesignBody upperRightLoadingNose = CreateCylinder(part, "Upper Inner Loading Nose (Right)",
                halfInnerSpan, 0, upperLoadingBaseZ,
                loadingNoseDia, width, loadingNoseHeight);
            NameSupportFace(upperRightLoadingNose, "Right_Upper", false, true);
        }
        ...
        /// 시편 Face 이름 지정
        /// </summary>
        private void NameSpecimenFaces(DesignBody specimenBody, DMA4PointBendingParameters p)
        {
            ...
            try
            {
                ...
                // 1. 시편 하단 면 (Z=0) - 하부 외부 지지점 접촉
                var bottomFaces = FaceNamingHelper.FindPlanarFaces(
                    specimenBody,
                    Direction.DirZ,
                    0.0,
                    FaceNamingHelper.Axis.Z);
                FaceNamingHelper.NameFaces(part, bottomFaces, "DMA_4PT_Specimen_Bottom_Contact");

                // 2. 시편 상단 면 (Z=thickness) - 상부 내부 로딩 노즈 접촉
                var topFaces = FaceNamingHelper.FindPlanarFaces(
                    specimenBody,
                    Direction.DirZ,
                    thick,
                    FaceNamingHelper.Axis.Z);
                FaceNamingHelper.NameFaces(part, topFaces, "DMA_4PT_Specimen_Top_Contact");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Failed to name 4PT specimen faces: {ex.Message}");
            }
        }
        ...
        private void NameSupportFace(DesignBody supportBody, string position, bool isLowerSupport, bool isLoadingNose)
        {
            ...
            try
            {
                ...
                if (topFace != null)
                {
                    ...
                    FaceNamingHelper.NameFace(part, topFace, $"DMA_4PT_{prefix}_{position}_Top");
                }
                ...
                {
                    string prefix = isLoadingNose ? "LoadingNose" : "Support";
                    FaceNamingHelper.NameFace(part, bottomFace, $"DMA_4PT_{prefix}_{position}_Bottom");
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Failed to name 4PT support faces: {ex.Message}");
            }
        }
    }
}
