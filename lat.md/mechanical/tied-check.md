...
`Tied Check` 툴바 버튼 → `show_tied_check_dialog` → `TiedContactCheckDialog` (`[[Mechanical/MXSimulator/main.py#TiedContactCheckDialog]]`, 라인 3517). 모달 실행 → RBM 검출 → Tied 생성 → 모달 재실행의 4단계 작업.
...
`on_create_contacts` (라인 4124) 가 RBM 바디들에 대해 자동으로 Bonded Contact 를 만든다. 각 바디마다:

1. `find_best_contact_target()` — 인접한 후보 바디들 중 가장 가까운 것을 BB 거리 + face facing 으로 선택. 알고리즘 라인 3950.
2. `bounding_box_distance()` (라인 4363) + `are_faces_facing()` (라인 4386) 로 면 쌍의 적합성 평가.
3. `create_contact_for_body()` (라인 4193) 가 실제 Contact 객체 추가 + Behavior=Bonded 설정.

## Suppress / Restore

수동 디버깅 옵션. 사용자가 RBM 으로 의심되지만 자동 픽스가 어려운 바디를 일시적으로 suppress 했다가 모달 재실행. 모든 suppress 해제도 한 클릭.

- `on_suppress` (라인 4425) — 선택 바디 suppress
- `on_restore_all` (라인 4469) — 모든 suppress 해제
- `on_rerun_modal` (라인 4463) — 모달 재실행
