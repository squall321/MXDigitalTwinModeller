...
$probe = "D:\MXDigitalTwinModeller\Test\RE_SelfTest\p0_from_empty_gate.py"
$out = "D:\MXDigitalTwinModeller\Test\RE_SelfTest\p0_gate_result.txt"
...
Get-Process SpaceClaim,ansyscl -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
if (Test-Path $out) { Write-Host "=== P0 RESULT ==="; Get-Content $out -Raw } else { Write-Host "NO RESULT" }
