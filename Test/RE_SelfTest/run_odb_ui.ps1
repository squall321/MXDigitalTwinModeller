# Verify ODB++ ribbon command registration + dialog construction headlessly.
$ErrorActionPreference = "Stop"
$sc   = "D:\Program Files\ANSYS Inc\ANSYS Student\v252\scdm\SpaceClaim.exe"
$gate = "D:\MXDigitalTwinModeller\Test\RE_SelfTest\probe_odb_ui.py"
$dir  = "D:\MXDigitalTwinModeller\Test\RE_SelfTest"
$done = Join-Path $dir "odb_ui_done.txt"
. (Join-Path $dir "_lic_watchdog.ps1")
Get-Process SpaceClaim, ansyscl -ErrorAction SilentlyContinue |
    Stop-Process -Force -ErrorAction SilentlyContinue
Start-Sleep -Seconds 1
Remove-Item $done, (Join-Path $dir "odb_ui_mark.txt") -ErrorAction SilentlyContinue
...
Write-Host ("odb_ui finished in {0:F1}s" -f (((Get-Date) - $start).TotalSeconds))
Get-Process SpaceClaim, ansyscl -ErrorAction SilentlyContinue |
    Stop-Process -Force -ErrorAction SilentlyContinue
if (Test-Path (Join-Path $dir "odb_ui_mark.txt")) { Get-Content (Join-Path $dir "odb_ui_mark.txt") }
