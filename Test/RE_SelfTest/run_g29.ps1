# Headless gate for the odb++ layer subdivision (g29_odblayers.py).
...
$gate = "D:\MXDigitalTwinModeller\Test\RE_SelfTest\g29_odblayers.py"
$dir  = "D:\MXDigitalTwinModeller\Test\RE_SelfTest"
$done = Join-Path $dir "g29_done.txt"
. (Join-Path $dir "_lic_watchdog.ps1")
Get-Process SpaceClaim, ansyscl -ErrorAction SilentlyContinue |
    Stop-Process -Force -ErrorAction SilentlyContinue
Start-Sleep -Seconds 1
Remove-Item $done, (Join-Path $dir "g29_mark.txt"), (Join-Path $dir "g29_result.txt") -ErrorAction SilentlyContinue
...
Write-Host ("g29 finished in {0:F1}s" -f (((Get-Date) - $start).TotalSeconds))
Get-Process SpaceClaim, ansyscl -ErrorAction SilentlyContinue |
    Stop-Process -Force -ErrorAction SilentlyContinue

if (Test-Path (Join-Path $dir "g29_result.txt")) {
    Write-Host "---- g29_result ----"
    Get-Content (Join-Path $dir "g29_result.txt")
} else {
    Write-Host "NO RESULT FILE - marks:"
    if (Test-Path (Join-Path $dir "g29_mark.txt")) { Get-Content (Join-Path $dir "g29_mark.txt") }
}
