...
{
    /// <summary>
    /// DMA 4점 굽힘시험 시편 생성 대화창 (Modeless with Preview)
    /// </summary>
    public partial class DMA4PointBendingDialog : Form
    {
        private readonly DMASpecimenFactory factory;
        private readonly DMA4PointBendingService modelingService;
        private Part activePart;
        private DesignBody previewBody;
        private List<DesignBody> previewFixtures; // 부자재 추적

        public DMA4PointBendingParameters Parameters { get; private set; }

        public DMA4PointBendingDialog(Part part)
        {
            InitializeComponent();
            factory = new DMASpecimenFactory();
            modelingService = new DMA4PointBendingService();
            activePart = part;
            Parameters = new DMA4PointBendingParameters();
            ...
            // 대화창이 닫힐 때 미리보기 정리
            this.FormClosing += DMA4PointBendingDialog_FormClosing;
            ...
            numOuterSpan.ValueChanged += (s, e) => UpdateWarnings();
            numInnerSpan.ValueChanged += (s, e) => UpdateWarnings();
        ...
        private void BindParametersToUI()
        {
            ...
            numOuterSpan.Value = (decimal)Parameters.OuterSpan;
            numInnerSpan.Value = (decimal)Parameters.InnerSpan;
            ...
            // 설명 업데이트 (Outer/Inner Span 비율 표시)
            UpdateDescription();
        ...
        private void UpdateDescription()
        {
            ...
            lblDescription.Text = factory.GetSpecimenTypeDescription(Parameters.SpecimenType) +
                $" (Outer/Inner = {ratio:F1}:1, 권장: 2:1 ~ 3:1)";
        }
        ...
        private void ReadParametersFromUI()
        {
            ...
            Parameters.OuterSpan = (double)numOuterSpan.Value;
            Parameters.InnerSpan = (double)numInnerSpan.Value;
        ...
        private void numOuterSpan_ValueChanged(object sender, EventArgs e)
        {
            UpdateDescription();
        }

        private void numInnerSpan_ValueChanged(object sender, EventArgs e)
        {
            UpdateDescription();
        }
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                WriteBlock.ExecuteTask("DMA Preview", () =>
                {
                    ...
                    // 시편 + 부자재 생성
                    previewBody = modelingService.Create4PointBendingSpecimen(activePart, Parameters);
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (previewBody == null)
                {
                    ...
                    {
                        modelingService.Create4PointBendingSpecimen(activePart, Parameters);
                    });
                }
                else
                {
        ...
        /// 폼 닫힐 때 미리보기 정리
        /// </summary>
        private void DMA4PointBendingDialog_FormClosing(object sender, FormClosingEventArgs e)
        {
